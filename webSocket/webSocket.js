...
class WebSocket {
    ...
    socketMessage () {
        return async res => {
			...
			// 开盒结果通知
                ...
                global._logger.writeLogs(`开盒结果通知。服务器消息：`, JSON.stringify(res.data));
			// 系统消息
			...
			// 个人中心红点
				...
				if(uni.showTabBarRedDot) {
					uni.showTabBarRedDot({index: 4, fail: function(err) {
						// 本地缓存添加红点事件
	...
	wallSocketCallback(xmlDom, type) {
		...
		let swWsObj = {
			...
			nick: arrayGetValue(xmlDom, "name", "nick", "value").length > 4 ? arrayGetValue(xmlDom, "name", "nick", "value").slice(0, 4)+"..." : arrayGetValue(xmlDom, "name", "nick", "value"),
	...
	gameSocketCallback(xmlDom, type) {
		...
		let _soldNum = arrayGetValue(xmlDom, "name", "soldNum", "value") === undefined ? -1 : arrayGetValue(xmlDom, "name", "soldNum", "value");													// 该房间剩余的格子数
		let _soldNum1 = arrayGetValue(xmlDom, "name", "soldNum1", "value") === undefined ? -1 : arrayGetValue(xmlDom, "name", "soldNum1", "value");													// 该系列剩余的格子数
		store.commit("setNewGameData", _newGameObj);
		uni.$emit("GAMEING_REFRESH", (_soldNum !== -1 || _soldNum1 !== -1) ? {
			soldNum: _soldNum,
			soldNum1: _soldNum1,
		} : null)
		global._logger.writeLogs(`新版游戏iq格式化_newGameObj:`, JSON.stringify(_newGameObj));
		global._logger.writeLogs(`新版游戏iq格式化_soldNum&_soldNum1: ${_soldNum}&${_soldNum1}`);
	}
	...
	openResultFunc(domParser) {
		...
		let newFlutterObj = {
			...
			nick: arrayGetValue(attr, "name", "nick", "value").length > 4 ? arrayGetValue(attr, "name", "nick", "value").slice(0, 4)+"..." : arrayGetValue(attr, "name", "nick", "value"),
	...
	messageFunc(domParser) {
		...
		let addMsgObj = {
			...
			buttonText: !!domParser.getElementsByTagName("button")[0] ? domParser.getElementsByTagName("button")[0].textContent : "",
			picture: !!domParser.getElementsByTagName("picture")[0] ? domParser.getElementsByTagName("picture")[0].textContent : "",
			url: `${!!domParser.getElementsByTagName("url")[0] ? domParser.getElementsByTagName("url")[0].textContent : ""}${!!domParser.getElementsByTagName("orderid")[0] ? "?submitId=" + domParser.getElementsByTagName("orderid")[0].textContent : ""}`,
		}
		global.messManager.addMess(addMsgObj);
		global.messManager.removeOldMess()
		// 服务器消息回执
		const returnMsg = `<message at="${arrayGetValue(attributesArr, "name", "at", "value")}" from="${arrayGetValue(attributesArr, "name", "to", "value")}" id="${arrayGetValue(attributesArr, "name", "id", "value")}" newstype="sysMsg" to="${arrayGetValue(attributesArr, "name", "from", "value")}" type="result"></message>`;
	...
	accountConflictFunc() {
		...
		const visitPages = ["pages/home/main", "pages/mallHome/main", "pages/boxArk/main", "pages/me/main", "pages/buyBlindBox/main", "pages/mallDetail/main"];
		// 302页面存在游客模式进入游客，否则302弹窗
		let pagesInfoRoute = "";
		// #ifdef MP-WEIXIN
			pagesInfoRoute = !!pagesInfo[0] ? pagesInfo[pagesInfo.length-1].route : ""; 
		// #endif
		// #ifdef MP-TOUTIAO
			pagesInfoRoute = !!pagesInfo[0] ? pagesInfo[pagesInfo.length-1].is.slice(1) : ""; 
		// #endif
		this.closeWS();
		if(!!pagesInfo[0] && visitPages.indexOf(pagesInfoRoute) == -1) {
			...
			store.commit("setJoinWeComState", 0)
			uni.setStorageSync("isSightseerStorage", "1")
			uni.setStorageSync("joinWeComStateStorage", "0");
			loginInit(0, 1, 1);
			Request.isLogin = 0;
		}
	}
